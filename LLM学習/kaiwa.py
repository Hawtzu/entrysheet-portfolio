...
from langchain.schema import SystemMessage, HumanMessage # SystemMessageもインポート
...
model_name = "gemini-2.5-flash" # または "gemini-2.5-flash", "gemini-1.5-pro"
...
try:
    response = llm.invoke(
        [
            SystemMessage(content="あなたは日本の地理に詳しい物知り博士です"), # システムメッセージ
            HumanMessage(content="日本で2番目に高い山は？") # ユーザーメッセージ
        ]
    )
    # 結果は .content で取得
